...
import { getSuggestions } from '~/services/suggestions'; // Import the suggestion service
import Animated, { 
  useAnimatedStyle, 
  useSharedValue,
  withSpring 
...
// const marker = Mapbox.AnimatedPoint
...
const MapScreen = () => {
  ...
  const infoBoxVisible = useSharedValue(0);


  useEffect(() => {
    if (distance) {
      infoBoxVisible.value = withSpring(1);
    } else {
      infoBoxVisible.value = withSpring(0);
    }
  }, [distance]);
  ...
  const [proximity] = useState<any>([37.6872, -0.1326]); // Example coordinates (Chuka, Kenya)
  const [origin] = useState<any>([37.6872, -0.1326]); // Example origin coordinates
  ...
  const snapPoints = useMemo(() => ['50%', '75%', '80%'], []); // Adjust these percentages as needed


    // Fetch suggestions when user types in the "from" input
    const handleFromLocationChange = async (text: string) => {
      setFromLocation(text);
      if (text.length > 2) { // Fetch suggestions after typing 3 or more characters
        setIsLoading(true);
        const suggestions = await getSuggestions(text, accessToken, proximity, origin);
        setFromSuggestions(suggestions);
        setIsLoading(false);
      } else {
        setFromSuggestions([]); // Clear suggestions if input is too short
      }
    };

      // Fetch suggestions when user types in the "to" input
  const handleToLocationChange = async (text: string) => {
    setToLocation(text);
    if (text.length > 2) {
      setIsLoading(true);
      const suggestions = await getSuggestions(text, accessToken, proximity, origin);
  ...
  // Handle suggestion selection and populate the input field
  const handleSuggestionSelect = (suggestion: any, type: 'from' | 'to') => {
    if (type === 'from') {
 ...
 />
      ...
      <Animated.View style={[styles.infoBox, infoBoxAnimatedStyle]}>
      ...
      >
        ...
        {/* Render Suggestions for From Location */}
        {fromSuggestions.length > 0 && (
          ...
          <View style={styles.inputContainer}>
            <MaterialIcons name="location-searching" size={24} color="purple" />
            <BottomSheetTextInput
           ...
           {/* Render Suggestions for To Location */}
        ...
        <TouchableOpacity style={[styles.directionButton, isLoading && styles.directionButtonDisabled]} onPress={handleGetDirections} disabled={isLoading}>
          {isLoading ? (
            <ActivityIndicator color="#fd1010" />
          ) : (
            <>
...
const styles = StyleSheet.create({
  ...
  suggestionList: {
    maxHeight: 200, // Optional: Adjust height to fit within the BottomSheet
...
//     const a = Math.sin(Δφ / 2) * Math.sin(Δφ / 2) + Math.cos(φ1) * Math.cos(φ2) * Math.sin(Δλ / 2) * Math.sin(Δλ / 2);
//     const c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
//     const distanceKm = (R * c) / 1000; // in kilometers
//     const durationHrs = distanceKm / AVERAGE_SPEED_KMH;
...
//         <Camera ref={cameraRef} followUser Location followZoomLevel={INITIAL_ZOOM} animationMode="flyTo" />
...
//       <BottomSheet ref={bottomSheetRef} index={2} snapPoints={snapPoints} backdropComponent={renderBackdrop} enablePanDownToClose>
...
//   locationButton: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#FF9800', padding: 12, borderRadius: 8, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.3, shadowRadius: 4 },
//   continueButton: { marginTop: 10, backgroundColor: '#4CAF50', padding: 12, borderRadius: 8, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.3, shadowRadius: 4 },
//   buttonText: { color: '#fff', fontSize: 16 },
//   infoBox: { position: 'absolute', top: 20, left: 20, padding: 10, backgroundColor: 'rgba(0,0,0,0.6)', borderRadius: 8 },
//   infoText: { color: '#fff', fontSize: 14 },
//   sheetContent: { paddingHorizontal: 20, paddingVertical: 10, backgroundColor: '#fff', borderTopLeftRadius: 20, borderTopRightRadius: 20 },
//   inputLabel: { fontSize: 16, marginBottom: 5, color: '#333' },
//   input: { padding: 10, borderColor: '#ddd', borderWidth: 1, borderRadius: 5, marginBottom: 10, backgroundColor: '#f9f9f9' },
...
// // // // // // // // // // // // // // //                     {/* Enter Location Button */}
// // // // // // // // // // // // // // //                     <TouchableOpacity style={styles.enterLocationButton} onPress={handleOpenPress}>
// // // // // // // // // // // // // // //                         <Text style={styles.buttonText}>ion</Text>
// // // // // // // // // // // // // // //                         {/* <AntDesign name="arrowright" size={24} color="white" /> */}
// // // // // // // // // // // // // // //                     </TouchableOpacity>
